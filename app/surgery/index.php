<section class="py-4">
    <div class="container">
        <h3 class="fw-bolder text-center">Surgery List</h3>
        <center>
        ...
        <table class="table table-striped table-bordered dt-init">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-center">Surgery Detail</th>
...
<script>
    $(function(){
        $('.dt-init').DataTable({
            ...
            serverSide: true,
            ajax: {
                url:"../classes/Master.php?f=dt_surgery",
                method:"POST"
            },
            columns: [{
                    data: 'no',
                ...
                },
                {
                    ...
                    render:function(data, type, row, meta){
                        return '<p class="m-0 text-truncate w-100">'+((row.surgery))+'</p>';
                        },
                    width:"20%"
                },
                {
                    className: 'py-1 px-2', 
                    render:function(data, type, row, meta){
                        return '<p class="m-0 text-truncate w-100">'+((row.description))+'...</p>';
                        },
                    width:"20%"
                },
                {
                    ...
                    render:function(data, type, row, meta){
                        if(row.status == 1)
                            return '<span class="badge bg-primary bg-gradient px-3 rounded-pill">Not Completed</span>';
                        else
                            return '<span class="badge bg-secondary bg-gradient px-3 rounded-pill">Completed</span>';
                    },
                    width:"10%"
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        el.find('.edit_data').attr("href","./?page=surgery/manage_surgery&id="+row.id)
                        el.find('.view_data').attr("href","./?page=surgery/view_details&id="+row.id)
            ...
            drawCallback: function(settings) {
                ...
                $('.delete_data').click(function(){
                    _conf("Are you sure to delete <b>"+$(this).attr('data-name')+"</b> from list?","delete_surgery",[$(this).attr('data-id')])
                })
            },
            language:{
                oPaginate: {
