...
Class Master extends DBConnection {
	...
	function save_surgery(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `surgery_list` set {$data}";
		}else{
			$sql = "UPDATE `surgery_list` set {$data} where id = '{$id}'";
		}
		$save = $this->conn->query($sql);
		if($save){
			$sid= empty($id) ? $this->conn->insert_id : $id ;
			$resp['sid'] = $sid;
			if(empty($id))
				$this->settings->set_flashdata('success',"Surgery has been added successfully.");
			else
				$this->settings->set_flashdata('success',"Surgery has been updated.");
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Saving Surgery failed';
			$resp['error'] = $this->conn->error;
		}
		return json_encode($resp);
	}
	function delete_surgery(){
		extract($_POST);
		$delete = $this->conn->query("DELETE FROM `surgery_list` where id = '{$id}' ");
		if($delete){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Surgery has been deleted successfully");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
	...
	function dt_surgery(){
		...
		$totalCount = $this->conn->query("SELECT * FROM `surgery_list` where  user_id = '{$this->settings->userdata('id')}' ")->num_rows;
		$search_where = "";
		$columns_arr = array("unix_timestamp(created_at)",
							"unix_timestamp(date)",
							"surgery",
							"");
		if(!empty($search['value'])){
			$search_where .= "surgery LIKE '%{$search['value']}%' ";
			$search_where .= " OR date_format(date,'%M %d, %Y') LIKE '%{$search['value']}%' ";
			$search_where = " and ({$search_where}) ";
		}
		$query = $this->conn->query("SELECT * FROM `surgery_list` where user_id = '{$this->settings->userdata('id')}' {$search_where}  ");
		$recordsFilterCount = $this->conn->query("SELECT * FROM `surgery_list`  where  user_id = '{$this->settings->userdata('id')}'  {$search_where} ")->num_rows;
...
switch ($action) {
	...
	break;
	case 'save_surgery':
		echo $Master->save_surgery();
	break;
	case 'delete_surgery':
		echo $Master->delete_surgery();
	break;
	case 'dt_surgery':
		echo $Master->dt_surgery();
	break;
	default:
		// echo $sysset->index();
		break;
